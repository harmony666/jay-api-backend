package cn.ichensw.jayapigateway.filter;
...
import cn.ichensw.jayapicommon.service.InnerUserInterfaceInfoService;
import cn.ichensw.jayapicommon.service.InnerUserService;
import cn.ichensw.jayapigateway.exception.BusinessException;
import cn.ichensw.jayclientsdk.utils.SignUtils;
...
public class CustomGlobalFilter implements GlobalFilter, Ordered {
    ...
    private static final String DYE_DATA_VALUE = "jay";
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        // 5. 请求转发，调用模拟接口
        // 6. 响应日志
        return handleResponse(exchange, chain, interfaceInfo.getId(), invokeUser.getId(),nonce);
    }
                        ...
                        if (body instanceof Flux) {
                            ...
                            return super.writeWith(
                                    fluxBody.map(dataBuffer -> {
                                        ...
                                        try {
                                            postHandler(exchange.getResponse(), interfaceInfoId, userId, nonce);
                                        } catch (Exception e) {
    ...
    private void postHandler(ServerHttpResponse response, Long interfaceInfoId, Long userId, String nonce) {
        ...
        if (response.getStatusCode() == HttpStatus.OK) {
            CompletableFuture.runAsync(() -> {
                if (lock.tryLock()) {
                    try {
                        addInterfaceNum(interfaceInfoId, userId, nonce);
                    } finally {
                        lock.unlock();
                    }
                }
    ...
    private void addInterfaceNum(Long interfaceInfoId, Long userId,String nonce) {
